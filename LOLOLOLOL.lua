...
if not syn then game.Players.LocalPlayer:Kick("Exploit not supported")
else print("exploit supported")
end
...
getgenv().UILibrary = loadstring(game:HttpGet("https://raw.githubusercontent.com/CatzCode/cattoware/main/libraries/ui.lua"))()
...
getgenv().PlrFinder = function(PlayerString)
	if PlayerString then
		...
		for i = 1,#PlayerTable do
			if PlayerTable[i].DisplayName:lower():sub(1,#PlayerString) == PlayerString or PlayerTable[i].Name:lower():sub(1,#PlayerString) == PlayerString  then
	...
	elseif Name:lower() == "all" then
		...
		for _, FP in next, GP do
			if FP ~= tostring(LP) and Name:lower() == FP.Name:sub(1, string.len(Name)):lower() then
				table.insert(Inserted, FP);return FP
			end
		end
	end
	return Inserted
...
local Flying, Aimlock, Blink, Camlock, TpBypass, AntiFe, Noclip,InfStam = false, false, false, false, false, false, false,false
local FlyMethod = "New"
local AntiGH = false

local AimVelocity, Flyspeed, Blinkspeed = 5, 10, 2
local AimPart, AimlockPlayer, CamlockPlayer, AimMode,CamPart,FirstFly = "Torso", nil, nil, "OldVelocity","Torso",false
...
getgenv().Notify = function(title,description,image,time)
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 40, 40)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(20, 20, 20))}
	...
	wait(time)
	FadeObject(BackFrame);FadeObject(Shadow);FadeObject(Desc);FadeObject(Title)
...
local AirWalk = NewIns("Part");	AirWalk.Anchored = true; AirWalk.Size = Vector3(20,1,20); AirWalk.Transparency = 1;AirWalk.Name = "AirWalk";
...
getgenv().ToggleAntiSit = function()
	...
	if AntiSit == false then
		for _,v in next, game.CoreGui:GetDescendants() do
			if v:IsA("Seat") then v.Parent = Workspace
			end
		end
	end
end
...
USI.InputBegan:Connect(function(key)
	if USI:GetFocusedTextBox() then return end
...
AimlockCheck.TextColor3 = Color3.fromRGB(255, 255, 255)
...
FlyCheck.TextColor3 = Color3.fromRGB(255, 255, 255)
...
TargetCheck.TextColor3 = Color3.fromRGB(255, 255, 255)
...
NoClipCheck.TextColor3 = Color3.fromRGB(255, 255, 255)
...
HealthCheck.TextColor3 = Color3.fromRGB(255, 255, 255)
...
getgenv().ShootPlayerPos = function()
	...
	if AimPart and AimMode == "NewVelocity" then
		AimPos = Funny.CFrame + Funny.Velocity / AimVelocity + Funny.RotVelocity / math.huge
...
getgenv().Fly = function()
	...
	while Flying and Character:FindFirstChild'Humanoid' and Character.Humanoid.Health > 0 and wait() do
		if KeysTable['W'] then Table1['W'] = Flyspeed else Table1['W'] = 0 end
		if KeysTable['A'] then Table1['A'] = -Flyspeed else Table1['A'] = 0 end
		if KeysTable['S'] then Table1['S'] = -Flyspeed else Table1['S'] = 0 end
		if KeysTable['D'] then Table1['D'] = Flyspeed else Table1['D'] = 0 end
		if (Table1['W'] + Table1['S']) ~= 0 or (Table1['A'] + Table1['D']) ~= 0 then
			BodyVelocity.Velocity = ((Workspace.CurrentCamera.CoordinateFrame.lookVector * (Table1['W'] + Table1['S'])) + ((Workspace.CurrentCamera.CoordinateFrame * CFrame(Table1['A'] + Table1['D'], (Table1['W'] + Table1['S']) * 0.2, 0).p) - Workspace.CurrentCamera.CoordinateFrame.p)) * 50
		else
			BodyVelocity.Velocity = Vector3(0,0.1,0)
		end
...
getgenv().togglefly = function()
	...
	while Flying == true and LP and LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health ~= 0 and
	...
	if AntiGH == true then
		if LP and LP.Character and LP.Character:FindFirstChildOfClass"Humanoid" then
			if Changed == Enum.HumanoidStateType.FallingDown or Changed == Enum.HumanoidStateType.PlatformStanding then
				LP.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Running);LP.Character.Humanoid.PlatformStand = false -- Credits to Aidez for this part
			end
		end
	end
end
...
if Flying and FlyMethod == "New" then Fly() end
...
do
	...
	local NameCallFunc = function(self, ...)
		...
		if (Method == "WaitForChild" or Method == "FindFirstChild") and getcaller() ~= script and TpBypass and Args[1] == "HumanoidRootPart"
		then Args[1] = "Torso"
			return NameCall(self, unpack(Args))
		end
...
RunService.Stepped:Connect(
	function()
		...
		if
			Camlock ~= false and CamlockPlayer and CamlockPlayer.Character and
		...
		if Flying and LP.Character then
			if LP.Character and LP.Character:FindFirstChild("Humanoid") then
				...
				LP.Character.Humanoid.PlatformStand = false;LP.Character.Humanoid.Sit = false
...
end,"airwalk",{}," only here for a fly!","")
...
if LP.Name ~= LP.DisplayName then
	UILibrary:CreateWatermark("xen.systems | " .. game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name .. " | ".. LP.DisplayName.."/"..LP.Name)
elseif LP.Name == LP.DisplayName then
	UILibrary:CreateWatermark("xen.systems | " .. game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name .. " | ".. LP.Name)
end
...
local v_00 = valuessector:AddDropdown("Flymode",{"Old","New"},false,false,function(Name)
	if Name == "Old" then
		FlyMethod = "Old"
...
local v_0 = valuessector:AddSlider("FlySpeed",1,8,20,false,function(value)
	Flyspeed = value
end)
local v_1 = valuessector:AddSlider("Blinkspeed",1,8,20,false,function(value)
	Blinkspeed = value
end)
...
miscstuff:AddButton("Get Game Last Updated",function()
	...
	Notify("xen.systems v2",ConvertTimeStamp(update_stamp),"",2)

end)

miscstuff:AddButton("Print Commands",function()
...
spamsector:AddToggle("Spamming Enabled",default,function(state)
	Spamming = state
...
spamsector:AddSlider("Spam Delay",0,1,5,false,function(value)
	SpamDelay = value
end)
spamsector:AddTextbox("Spam Message",false,function(text)
	SpamMessage = text
end)


local A_0 = aimbotsector:AddToggle("Aimlock",default,function(state)
	Aimlock = state


end)
local A_01 = A_0:AddKeybind(Enum.KeyCode.T,function()
	Notify("xen.systems v2","Aimlocking: ".. Aimlock,"",2)
end)
if game.PlaceId ~= tostring(Streets) then
	local A_2 = aimbotsector:AddDropdown("AimMode",{"NewVelocity","OldVelocity"},false,false,function(Name)
...
local A_20 = aimbotsector:AddDropdown("AimPart",{"Head","Torso","HumanoidRootPart"},false,false,function(Name)
	if Name == "Head" then
		AimPart = "Head"
...
local A_3 = aimbotsector:AddDropdown("CamPart",{"Head","Torso","HumanoidRootPart"},false,false,function(Name)
	if Name == "Head" then
		CamPart = "Head"
	elseif Name == "Torso" then
		CamPart = "Torso"
...
local A_4 = aimbotsector:AddSlider("AimVelocity",1,7,10,false,function(value)
	AimVelocity = value
end)
...
local M_1 = maintab:AddToggle("Noclipping",default,function(state)
	Noclip = state
	Notify("xen.systems v2","Noclipping: " .. Noclip,"",2)
end)


local M_100 = maintab:AddToggle("Disable Animations",default,function(state)
	LP.Character:FindFirstChild("Animate").Disabled = state
end)

local M_01 = M_1:AddKeybind(Enum.KeyCode.X,function()

end)
local M_2 = maintab:AddToggle("Blinking",default,function(state)
	Blink = state
end)

local M_3 = maintab:AddToggle("AntiSit",default,function(state)
	AntiSit = state
	ToggleAntiSit()
end)

local M_4 = maintab:AddToggle("Prison TPBypass",default,function(state)
...
local M_5 = maintab:AddToggle("Anti Groundhit",default,function(state)
	AntiGH = state
end)
if game.PlaceId == tonumber(Streets) then
	local M_6 = maintab:AddToggle("Infinite Stamina",default,function(state)
		InfStam = state
		local Stam = LP.Character:FindFirstChild("Stamina")
		LP.Character:FindFirstChild("Stamina") :GetPropertyChangedSignal("Value"):Connect(function()
...
if game.PlaceId == tonumber(Prison) then
	local M_6 = maintab:AddToggle("Infinite Stamina",default,function(state)
		...
		LP.Backpack:FindFirstChild("ServerTraits").Stann:GetPropertyChangedSignal("Value"):Connect(function()
...
while CashFarming do wait(CycleWaitTime)
	for _,v in next, Workspace:GetDescendants() do
		if v:IsA("MeshPart") and v.MeshId == tostring(ID) then
			print('there is cash')
			local Teleport = TweenService:Create(LP.Character.HumanoidRootPart, TweenInfo.new(TeleportSpeed + .1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {CFrame = v.CFrame})
			Teleport:Play()

		end
	end
end
maintab:AddToggle("WIP AUTOFARM",default,function(state)
...
maintab:AddButton("Kick Self",function()
	LP:Kick()
end)
maintab:AddButton("Copy Discord Invite",function()
	setclipboard("https://discord.gg/rDsKjcnb")
end)
maintab:AddButton("Copy Owners Discord Tag",function()
...
local flybox = mainbindsector:AddTextbox("Fly Key",false,function(l)
	FlyKey = l
end)

local getammobox = keybindsector:AddTextbox("Buy Ammo Key",false,function(l)
	GetAmmoKey = l
end)
local getglockbox = keybindsector:AddTextbox("Buy Glock Key",false,function(l)
	GetGlockKey = l
end)
local getuzibox = keybindsector:AddTextbox("Buy Uzi Key",false,function(l)
	GetUziKey = l
end)
local getfoodbox = keybindsector:AddTextbox("Buy Food Key",false,function(l)
	GetHealKey = l
end)
...
local lel1= sector0:AddToggle("Enabled",default,function(state)
	if state == true then
		ESP:Toggle(true)
	end
	if state == false then
...
local lel2= sector0:AddToggle("Boxes",default,function(state)
	ESP.Boxes = state
end)
local lel3= sector0:AddToggle("Tracers",default,function(state)
	ESP.Tracers = state
end)
sector0:AddSeperator("ESP Info")
local lel4= sector0:AddToggle("ShowInfo",default,function(state)
	ESP.ShowInfo = state
end)
local lel5= sector0:AddToggle("Show Health",default,function(state)
	ESP.Info.Health = state
end)
local lel6= sector0:AddToggle("Show Names",default,function(state)
	ESP.Info.Name = state
	ESP.Names = state
end)
local lel7= sector0:AddToggle("Show Distance",default,function(state)
	ESP.Info.Distance = state
end)
local lel8= sector0:AddToggle("Show Weapon Info",default,function(state)
...
local L_1 = LightingSector:AddSlider("ClockTime/Time",0,7,14,false,function(value)
	ClockTime = value
end)

LightingSector:AddButton("Fix Brightness",function()
	game:GetService("Lighting").ColorCorrection.Brightness = 0.04
end)

local L_2 = LightingSector:AddColorpicker("Light Hue/Ambience Color",Color3.fromRGB(255,255,255),function(Color)
	local C3 = Color
...
SkySector:AddButton("Summer/Spring Sky",function()
	SummerSkyBox()
end)

SkySector:AddButton("Fall/Halloween Sky",function()
	FallSky()
end)

SkySector:AddButton("Winter/Christmas Sky",function()
	WinterSky()
end)
SkySector:AddButton("Prison/BanlandTS Sky",function()
	PrisonSky()
end)
SkySector:AddButton("Retro/RetroWave/90's Sky",function()
	RetroSky()
end)
SkySector:AddButton("Galaxy/Space Sky",function()
	GalaxySky()
end)



MapSector:AddButton("Summer/Spring",function()
...
MapSector:AddButton("Winter/Christmas",function()
	for _,v in next, Workspace:GetChildren() do
		if v.Name == "ChangableSeasonPart" then
...
MapSector:AddButton("Fall/Halloween",function()
	for _,v in next, Workspace:GetChildren() do
		if v.Name == "ChangableSeasonPart" then
...
local B_1 = BulletSector:AddSlider("BulletTransparency",0,0.5,1,false,function(value)
	BulletTransparency = value
end)


local B_2 = BulletSector:AddSlider("BulletTrailTime",1,1,10,false,function(value)
	BulletTrailTime = value
end)

local B_3 = BulletSector:AddColorpicker("Bullet Color",Color3.fromRGB(255,255,255),function(Color)
	local C3 = Color
	local r, g, b = math.round(C3.R*255), math.round(C3.G*255), math.round(C3.B*255)
	local RGB_Color = Color3.fromRGB(r,g,b)
